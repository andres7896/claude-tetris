...
- **Power-ups**: every `POWERUP_EVERY` lines cleared, `clearLines()` queues one in `pendingPowerUps`; `spawn()` drains that queue into `next` via `randomPowerUp()` (a 1×1 piece tagged `power: 'bomb' | 'ray' | 'tint' | 'gravity' | 'freeze'`) instead of a normal tetromino. It moves/rotates/drops like any piece but never `merge()`s into the board — `lockPiece()` routes it to `activatePowerUp()` instead, which mutates `board` directly (3×3 clear, row+column clear, recolor to `WILD` cells, per-column compaction, or setting `freezeRemaining`) and scores destroyed cells via `BLOCK_BONUS * level`. `isRowFull()` (used by `clearLines()`) treats a row as complete if at most one cell is empty and the row contains a `WILD` cell, so Tint-created wildcards can complete near-full rows. `freezeRemaining` (ms, ticked down in `loop()`) suspends only the automatic `dropAccum` advance, not player input. Transient visuals (flash cells, effect-name banner) live in the `effects` array, decremented each frame and drawn by `drawEffects()`.
...
`COLS`, `ROWS`, `BLOCK` (cell size in px), `COLORS`, `LINE_SCORES`, initial `dropInterval`, `POWERUP_EVERY` (lines between power-ups), `FREEZE_MS` (Freeze duration), `BLOCK_BONUS` (points per cell destroyed by Bomb/Ray). If `COLS`/`ROWS`/`BLOCK` change, update the `#board` canvas `width`/`height` in `index.html` to match (`COLS × BLOCK` by `ROWS × BLOCK`).
